...
describe('IndexedDB v2 Eternal Terminal state', () => {
  ...
  it('deferred flush keeps the highest rxSequence across pending checkpoints', async () => {
    await saveEtSession(record());
    ...
    await checkpointEtOutput('local-session', 3, new Uint8Array([3]), { ...hint, rxSequence: 2 });
    await flushEtSessionCheckpoint('local-session');
    expect(await getEtSession('local-session')).toMatchObject({ rxSequence: 3 });
  });
